    ...
    if (response.status === 401 && !isRetry) {
      ...
      try {
        ...
        if (refreshResult.access_token) {
          ...
          console.log(
            "apiRequest: Token refreshed, retrying original request...",
          );
...
export const authAPI = {
  async login(email: string, password: string) {
    return apiRequest<{ access_token: string; message: string }>(
      "/auth/login",
      {
        method: "POST",
  ...
  async refreshToken() {
    console.log(
      "refreshToken: Attempting to refresh token using HttpOnly cookie...",
    );
    try {
...
export const profileAPI = {
  ...
  async setDefaultProfile(profileId: string) {
    return apiRequest<{ message: string }>(
      `/user/search-profile/default/${profileId}`,
      {
        method: "PUT",
      },
    );
...
export const chatAPI = {
  ...
  async createStreamingChatSession(profileId: string) {
    return apiRequest<any>(
      `/llm/prompt-response/chat/sessions/stream/${profileId}`,
      {
        method: "POST",
      },
    );
  ...
  async sendStreamingMessage(sessionId: string, message: string) {
    ...
    return fetch(
      `${API_BASE_URL}/llm/prompt-response/chat/sessions/${sessionId}/messages/stream`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
export const listsAPI = {
  ...
  async removePOIFromItinerary(itineraryId: string, poiId: string) {
    return apiRequest<{ message: string }>(
      `/itineraries/${itineraryId}/items/${poiId}`,
      {
        method: "DELETE",
      },
    );
...
export const hotelsAPI = {
  ...
  async getNearbyHotels(lat: number, lng: number, radius?: number) {
    ...
    return apiRequest<any[]>(
      `/llm/prompt-response/city/hotel/nearby?${params}`,
    );
  },
...
export const restaurantsAPI = {
  async getRestaurantsByPreferences(preferences: any) {
    return apiRequest<any[]>(
      "/llm/prompt-response/city/restaurants/preferences",
      {
        method: "POST",
        body: JSON.stringify(preferences),
      },
  ...
  async getNearbyRestaurants(lat: number, lng: number, radius?: number) {
    ...
    return apiRequest<any[]>(
      `/llm/prompt-response/city/restaurants/nearby?${params}`,
    );
  },

  async getRestaurantDetails(restaurantId: string) {
    return apiRequest<any>(
      `/llm/prompt-response/city/restaurants/${restaurantId}`,
    );
  },
};
...
export const bookmarkAPI = {
  ...
  async removeItinerary(itineraryId: string) {
    return apiRequest<{ message: string }>(
      `/llm/prompt-response/bookmark/${itineraryId}`,
      {
        method: "DELETE",
      },
    );
