...
const isValidDomainType = (value: unknown): value is DomainType => {
    const validDomains: DomainType[] = ['general', 'itinerary', 'accommodation', 'dining', 'activities'];
...
const isValidStreamEventType = (value: unknown): value is StreamEventType => {
    const validTypes: StreamEventType[] = [
        'start', 'chunk', 'complete', 'error', 'city_data',
        'general_pois', 'itinerary', 'hotels', 'restaurants', 'activities', 'progress'
    ];
