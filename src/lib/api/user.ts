...
import { UserService } from '@buf/loci_loci-proto.bufbuild_es/proto/user_pb.js';
import {
  GetUserProfileRequestSchema,
  UpdateUserProfileRequestSchema,
  UpdateProfileParamsSchema,
} from '@buf/loci_loci-proto.bufbuild_es/proto/user_pb.js';
  ...
  return useQuery(() => ({
    ...
    queryFn: async (): Promise<UserProfileResponse> => {
      ...
      return {
        ...
        created_at: profile.createdAt ? new Date(Number(profile.createdAt.seconds) * 1000).toISOString() : '',
        updated_at: profile.updatedAt ? new Date(Number(profile.updatedAt.seconds) * 1000).toISOString() : '',
