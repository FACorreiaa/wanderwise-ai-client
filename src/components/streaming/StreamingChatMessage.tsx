...
export function StreamingChatMessage(props: StreamingChatMessageProps) {
  ...
  const processPartialData = (newData: any) => {
    ...
    if (newData.general_city_data || currentData.general_city_data) {
      const cityData =
        newData.general_city_data || currentData.general_city_data;
      if (cityData) {
        results.push({
          type: "city",
    ...
    if (newData.itinerary_response || currentData.itinerary_response) {
      const itinerary =
        newData.itinerary_response || currentData.itinerary_response;
      if (itinerary) {
        results.push({
          type: "itinerary",
    ...
    if (newData.accommodation_response || currentData.accommodation_response) {
      const hotels =
        newData.accommodation_response || currentData.accommodation_response;
    ...
    if (newData.dining_response || currentData.dining_response) {
      const restaurants =
        newData.dining_response || currentData.dining_response;
    ...
    if (newData.activities_response || currentData.activities_response) {
      const activities =
        newData.activities_response || currentData.activities_response;
                ...
                <div class="bg-gray-50 rounded-lg p-4 border border-gray-200 animate-fade-in">
                  <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1">
                      ...
                      <Show
                        when={result.type === "pois" && result.data?.length > 0}
                      >
                        <div class="space-y-1">
                      ...
                      >
                        <div class="text-xs text-gray-500 bg-white rounded px-2 py-1">
                          ...
                          {result.data.duration_days &&
                            ` (${result.data.duration_days} days)`}
                        </div>
                      </Show>
                    </div>
                  </div>
          ...
          >
            <div class="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg">
              <p class="text-sm text-green-700 font-medium">
                ✓ Your travel plan is ready! Click on any section above to
                explore in detail.
              </p>
            </div>
          </Show>
