...
export default function ActivityResults(props: ActivityResultsProps) {
  ...
  const getBudgetColor = (budget: string) => {
    ...
    if (budgetLower.includes("expensive") || budgetLower.includes("high"))
      return "text-red-600 dark:text-red-400";
    if (budgetLower.includes("moderate") || budgetLower.includes("medium"))
  ...
  const getCategoryEmoji = (category: string) => {
    ...
    if (categoryLower.includes("art") || categoryLower.includes("gallery"))
      return "🎨";
    if (categoryLower.includes("music") || categoryLower.includes("concert"))
      return "🎵";
    if (categoryLower.includes("sport") || categoryLower.includes("stadium"))
      return "⚽";
    if (categoryLower.includes("shopping")) return "🛍️";
    if (categoryLower.includes("tour") || categoryLower.includes("walking"))
      return "🚶‍♂️";
    if (
    ...
    <div class="space-y-3">
                ...
                <div class="flex items-center gap-2 ml-2 flex-shrink-0">
                  <Show when={activity.rating}>
                    <div class="flex items-center gap-1">
                      <Star
                        class={`w-3 h-3 fill-current ${getRatingColor(activity.rating!)}`}
                      />
                      <span
                        class={`text-xs font-medium ${getRatingColor(activity.rating!)}`}
                      >
                        {activity.rating}
                      </span>
                    </div>
      ...
      <Show
        when={
          props.limit &&
          !props.showToggle &&
          props.activities.length > props.limit
        }
      >
