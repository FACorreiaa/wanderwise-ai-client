...
const AuthLayout: Component<{ children: any; showBackButton?: boolean; onBack?: () => void }> = (
  ...
  return (
    <div
      class={`min-h-screen relative overflow-hidden ${shellBg()} ${isDark() ? "text-white" : "text-slate-900"}`}
    >
      <Show when={isDark()}>
      ...
      <div class="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-10 items-center">
          ...
          <div class="hidden lg:block">
            <div
              class={`rounded-3xl p-8 space-y-6 ${isDark() ? "glass-panel gradient-border shadow-[0_35px_120px_rgba(3,7,18,0.55)]" : "bg-white border border-slate-200 shadow-[0_25px_80px_rgba(15,23,42,0.12)]"}`}
            >
              ...
              <h1
                class={`text-4xl font-extrabold leading-tight ${isDark() ? "text-white" : "text-slate-900"}`}
              >
              ...
              <p
                class={`text-lg leading-relaxed ${isDark() ? "text-slate-200/85" : "text-slate-700"}`}
              >
              ...
              <div class="grid grid-cols-2 gap-3">
                ...
                >
                  {(item) => (
                    <div
                      class={`flex items-start gap-2 rounded-2xl p-3 ${isDark() ? "bg-white/5 border border-white/10" : "bg-slate-50 border border-slate-200"}`}
                    >
                      <FiCheck
                        class={`w-4 h-4 ${isDark() ? "text-emerald-300" : "text-emerald-600"} mt-0.5`}
                      />
                      <span class={`text-sm ${isDark() ? "text-slate-100" : "text-slate-800"}`}>
                        {item}
                      </span>
                    </div>
                  )}
          ...
          <div class="w-full max-w-xl mx-auto">
            ...
            <div class={`rounded-3xl p-5 sm:p-6 lg:p-7 ${cardBg()} ${cardShadow()}`}>
              <div class="flex items-center gap-2 mb-6">
                <div
                  class={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl ${isDark() ? "bg-white/15 border border-white/30 text-white shadow-[0_14px_32px_rgba(12,125,242,0.32)]" : "bg-emerald-50 border border-emerald-100 text-emerald-700 shadow-[0_12px_28px_rgba(16,185,129,0.25)]"} backdrop-blur flex items-center justify-center font-bold`}
                >
                  L
                </div>
            ...
            <div class={`text-center mt-4 sm:mt-6 text-xs sm:text-sm px-2 sm:px-0 ${textSoft()}`}>
              ...
              <a
                href="#"
                class={`${isDark() ? "text-emerald-200 hover:text-emerald-100" : "text-emerald-700 hover:text-emerald-600"} underline-offset-4`}
              >
                Terms
              </a>{" "}
              and{" "}
