...
export default function NearmePage() {
    ...
    const allPois = createMemo((): POIDetailedInfo[] => {
        ...
        if (Array.isArray(generalPois)) {
            pois.push(...generalPois.map((p: any) => ({
                ...p,
        ...
        if (Array.isArray(restaurants)) {
            pois.push(...restaurants.map((r: any) => ({
                ...r,
                category: 'restaurant',
            })));
        }
        ...
        if (Array.isArray(activities)) {
            pois.push(...activities.map((a: any) => ({
                ...a,
                category: 'activity',
            })));
        }
    ...
    const ListContent = (
        <div class="h-full overflow-y-auto p-4 md:p-6 bg-slate-50/50 dark:bg-slate-950/50 backdrop-blur-sm">
            <div class="max-w-3xl mx-auto pb-20">
                                    ...
                                    <For each={DISTANCE_OPTIONS}>
                                        {(option) => (
                                            <button
                                                ...
                                                class={`w-full px-4 py-2 text-left hover:bg-gray-100 dark:hover:bg-gray-700 first:rounded-t-lg last:rounded-b-lg ${selectedDistance() === option.value
                ...
                <Show when={isLoadingLocation()}>
                    <div class="mb-6 p-6 rounded-xl bg-blue-50 border border-blue-200 dark:bg-blue-900/20 dark:border-blue-900/50 flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-blue-600 dark:text-blue-300 opacity-80">This may take a few seconds</p>
                        </div>
                    </div>
                </Show>
                ...
                {/* Empty State */}
                <Show when={!isLoadingLocation() && !state.isStreaming && userLocation() && allPois().length === 0}>
