...
export default function RestaurantsPage() {
  ...
  const [streamingData, setStreamingData] = createSignal<DiningResponse | null>(
    null,
  );
    ...
    if (urlSessionId) {
      ...
      if (storedSessionData) {
        try {
          ...
          console.log(
            "Loading restaurants session data from URL parameters:",
            sessionData,
          );
          ...
          console.error(
            "Error parsing restaurants session data from URL:",
            error,
          );
        }
      }
      ...
      if (storedSession) {
        try {
          ...
          if (session.data && session.data.restaurants) {
            ...
            setFromChat(true);
            console.log(
              "Loaded dining data from session storage:",
              session.data,
            );
          } else {
  ...
  const restaurants = () => {
    ...
    if (
      streaming &&
      streaming.restaurants &&
      streaming.restaurants.length > 0
    ) {
  ...
  const convertRestaurantToDisplayFormat = (
  ...
  const filteredRestaurants = () => {
    return restaurants().filter((restaurant) => {
      ...
      if (
        ...
        !filters.cuisines.some((cuisine) => restaurantTags.includes(cuisine))
      )
        return false;
      // Price range filter
      if (
        filters.priceRange.length > 0 &&
        !filters.priceRange.includes(restaurantPriceRange)
      )
        return false;
      // Features filter
      if (
  ...
  const toggleFavorite = (restaurantName: string, restaurant: any) => {
    ...
    console.log("Toggle favorite for restaurant:", {
      restaurantName,
      isFavorite: isFavorite(restaurantName),
    });
                ...
                <div class="flex flex-wrap items-center gap-2 mb-2">
                  ...
                  <span
                    class={`px-3 py-1 rounded-full text-xs font-bold border ${
                      getPriceColor(restaurant.priceRange).includes("green")
                        ? "bg-green-50 text-green-700 border-green-200"
                        : getPriceColor(restaurant.priceRange).includes("blue")
                          ? "bg-blue-50 text-blue-700 border-blue-200"
                          : getPriceColor(restaurant.priceRange).includes(
                                "orange",
                              )
                  ...
                  <Show
                    when={restaurant.rating >= 4.0 && restaurant.rating < 4.5}
                  >
                  ...
                  <Show
                    when={
                      restaurant.features.includes("Vegetarian") ||
                      restaurant.features.includes("Vegan")
                    }
                  >
                  ...
                  <Show
                    when={
                      restaurant.features.includes("Outdoor") ||
                      restaurant.features.includes("Terrace")
                    }
                  >
                  ...
                  <Show
                    when={
                      restaurant.features.includes("Wine") ||
                      restaurant.features.includes("Bar")
                    }
                  >
                  ...
                  return (
                    <label class="flex items-center gap-2 text-sm cursor-pointer hover:bg-gray-50 p-1 rounded">
                      <input
                        ...
                        checked={activeFilters().cuisines.includes(
                          cuisine.label,
                        )}
  ...
  const getSearchCoordinates = () => {
    const streaming = streamingData();
    console.log(
      "🍽️ RESTAURANTS getSearchCoordinates - streaming data:",
      streaming,
    );
    ...
    if (
      ...
      if (firstRestaurant.latitude && firstRestaurant.longitude) {
        const coords = {
          lat:
            typeof firstRestaurant.latitude === "string"
              ? parseFloat(firstRestaurant.latitude)
              : firstRestaurant.latitude,
          lon:
            typeof firstRestaurant.longitude === "string"
              ? parseFloat(firstRestaurant.longitude)
            ...
            <div class="flex items-center gap-3">
              ...
              <div class="flex-1">
                ...
                <p class="text-xs text-green-700">
                  ...
                  {location.state?.originalMessage || "Restaurant search"}"
                </p>
              </div>
              <button
          ...
          <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h1 class="text-xl font-bold text-gray-900 dark:text-white sm:text-2xl">
                Restaurants in {displayLocation()}
              </h1>
              <p class="text-sm text-gray-600 dark:text-gray-300 mt-1 sm:text-base">
                {restaurants().length} dining recommendations
              </p>
            </div>
          ...
          <Show when={viewMode() === "list" || viewMode() === "split"}>
            <div class={viewMode() === "list" ? "col-span-full" : ""}>
              <div class="space-y-4">
                ...
                <Show
                  when={filteredRestaurants().length > 0}
                  fallback={
                    <div class="text-center py-12">
                      ...
                      <p class="text-gray-600 dark:text-gray-300 mb-4">
                        Start a new search from the home page to find
                        restaurants
                      </p>
                      <button
                    ...
                    onShareClick={(restaurant) => {
                      if (navigator.share) {
                        ...
                        navigator.clipboard.writeText(
                          `Check out ${restaurant.name}: ${restaurant.description_poi}`,
                        );
                      }
                    }}
                    favorites={
            ...
            <div class="p-4 sm:p-6">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 mb-4 text-sm">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <span class={getPriceColor(selectedRestaurant().priceRange)}>
              ...
              <div class="border-t pt-4">
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div class="text-sm text-gray-600">
                    ...
                    <p>
                      <strong>Website:</strong> {selectedRestaurant().website}
                    </p>
                  </div>
                  <div class="flex flex-col gap-2 sm:flex-row">
                    ...
                    >
                      ...
                      >
                        <Heart
                          class={`w-4 h-4 inline mr-1 ${isFavorite(selectedRestaurant().name) ? "fill-current" : ""}`}
                        />
                      </Show>
                      {!isAuthenticated()
                        ? "Login to Favorite"
